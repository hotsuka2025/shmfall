...
**shmfall** は WIN 形式（卜部 1994）の時系列データを、`shmdump` が生成するプレーンテキストストリームから読み取り、リアルタイムでウォーターフォール表示する Python ツールです。共有メモリを直接扱わず、`shmdump -tq` 等の標準出力をパイプで受け取って動作します。

本 README は shmfall.py の簡潔な使用案内です。詳細は下記文献を参照してください。
...
- 既定サンプリング周波数: `SAMPLING_RATE`（ソース参照）
- バッファ長: `MAX_SAMPLES = duration * SAMPLING_RATE`

## 前提（推奨環境）
- Ubuntu（本ドキュメントは Ubuntu を想定）
- Python 3.12
- 必要ライブラリ: `matplotlib`, `scipy`, `numpy`（プロジェクトにより他依存あり。`pyproject.toml` を参照）
...
- チャネルテーブルの列配置（ID や緯度・経度の列番号）は `shmfall.py` の期待に合わせてください（ソースが split() で解析している列番号に依存します）。
